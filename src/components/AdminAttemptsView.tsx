...
export default function AdminAttemptsView({ onBack }: Props) {
  ...
  useEffect(() => {
    const loadExams = async () => {
      try {
        const { data, error: queryError } = await supabase.from('exams').select('*').order('starts_at', { ascending: false });
        if (queryError) throw queryError;
        const rows = (data || []) as Exam[];
        setExams(rows);
    ...
    const loadAttempts = async () => {
      ...
      try {
        const { data, error: queryError } = await supabase
          .from('exam_attempts')
          .select('*, students (student_name, gmail, whatsapp_number, school)')
          .eq('exam_id', examId)
          .order('started_at', { ascending: false });
        if (queryError) throw queryError;
        setAttempts(((data || []) as ExamAttempt[]).sort((left, right) => Date.parse(right.started_at) - Date.parse(left.started_at)));
      } catch (loadError) {
  ...
  const duplicateValues = useMemo(() => {
    const values = (key: 'ip_address' | 'device_fingerprint') => {
      ...
      attempts.forEach((attempt) => {
        ...
        studentIds.add(attempt.student_id);
    ...
    <main className="min-h-screen bg-slate-900 px-4 py-8 text-white">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
          ...
          <select value={examId} onChange={(event) => setExamId(event.target.value)} disabled={loading && exams.length === 0} className="rounded-xl border border-white/10 bg-slate-800 px-4 py-3 text-white"><option value="">Select an exam</option>{exams.map((exam) => <option key={exam.id} value={exam.id}>{exam.exam_name}</option>)}</select>
        </div>
